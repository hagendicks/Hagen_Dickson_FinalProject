...
def signup_influencer():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO INFLUENCER (InfluencerName, Email, Password, Handle, Industry, Age, Gender, Field, Followers) VALUES (%s, %s, %s, %s, %s, 25, 'Other', 'General', 0)", (name, email, hashed_pw, handle, industry))
            conn.commit()
            return redirect(url_for('login'))
...
def signup_brand():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO BRAND (Brandname, Email, Password, PayPackage) VALUES (%s, %s, %s, %s)", (name, email, hashed_pw, budget))
            conn.commit()
            return redirect(url_for('login'))
        except Exception as e:
            return f"Database Error: {e}"
...
def api_update_status():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE INFLUENCER_CAMPAIGN ic JOIN CAMPAIGN c ON ic.CampaignID = c.CampaignID SET ic.CampaignStatus = %s WHERE ic.CAM_ID = %s AND c.BrandID = %s", (action, app_id, session['user_id']))
        conn.commit()
    ...
    try:
        ...
        if role == 'influencer':
            ...
            if name and handle and industry:
                cursor.execute("UPDATE INFLUENCER SET InfluencerName = %s, Handle = %s, Industry = %s WHERE InfluencerID = %s", (name, handle, industry, user_id))
            ...
            if name and email and budget:
                cursor.execute("UPDATE BRAND SET Brandname = %s, Email = %s, PayPackage = %s WHERE BrandID = %s", (name, email, budget, user_id))
                session['user_name'] = name
        conn.commit()
        return jsonify({'success': True, 'new_image': image_path, 'message': 'Profile updated successfully'})
    except Exception as e:
...
def influencer_dashboard():
    ...
    cursor.execute("""SELECT b.Brandname, c.CampaignName as CampaignType, ic.StartDate as DateApplied, ic.CampaignStatus as Status 
                      FROM INFLUENCER_CAMPAIGN ic JOIN CAMPAIGN c ON ic.CampaignID = c.CampaignID JOIN BRAND b ON c.BrandID = b.BrandID 
                      WHERE ic.InfluencerID = %s LIMIT 5""", (session['user_id'],))
    recent_apps = cursor.fetchall()
    cursor.execute("SELECT Industry FROM INFLUENCER WHERE InfluencerID = %s", (session['user_id'],))
    my_industry = cursor.fetchone()['Industry']
    recommendations = []
    if my_industry:
        cursor.execute("""SELECT c.CampaignID, c.CampaignName, b.Brandname, c.EndDate, b.PayPackage FROM CAMPAIGN c JOIN BRAND b ON c.BrandID = b.BrandID JOIN INDUSTRY i ON c.IndustryID = i.IndustryID 
                          WHERE i.Industryname = %s AND c.CampaignStatus = 'Active' AND c.CampaignID NOT IN (SELECT CampaignID FROM INFLUENCER_CAMPAIGN WHERE InfluencerID = %s) LIMIT 4""", (my_industry, session['user_id']))
...
def brand_dashboard():
    ...
    if active_industries:
        ...
        query = f"SELECT * FROM INFLUENCER WHERE Industry IN ({placeholders}) ORDER BY Followers DESC LIMIT 4"
...
def feed():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""SELECT c.CampaignID as ID, c.CampaignName as Title, c.StartDate as Date, b.Brandname as AuthorName, b.LogoPic as AuthorPic, b.BrandID as AuthorID, 'Brand' as AuthorType, 'campaign' as PostType, b.PayPackage, i.Industryname 
                      FROM CAMPAIGN c JOIN BRAND b ON c.BrandID = b.BrandID JOIN INDUSTRY i ON c.IndustryID = i.IndustryID WHERE c.CampaignStatus = 'Active'""")
    campaigns = cursor.fetchall()
    for c in campaigns:
        if type(c['Date']) is date: c['Date'] = datetime.combine(c['Date'], datetime.min.time())
    cursor.execute("""SELECT p.PostID as ID, p.Content as Title, p.CreatedAt as Date, CASE WHEN p.AuthorType = 'Brand' THEN b.Brandname ELSE i.InfluencerName END as AuthorName, CASE WHEN p.AuthorType = 'Brand' THEN b.LogoPic ELSE i.ProfilePic END as AuthorPic, p.AuthorID, p.AuthorType, 'post' as PostType, p.ImageURL as PayPackage, '' as Industryname 
                      FROM POSTS p LEFT JOIN BRAND b ON p.AuthorType = 'Brand' AND p.AuthorID = b.BrandID LEFT JOIN INFLUENCER i ON p.AuthorType = 'Influencer' AND p.AuthorID = i.InfluencerID""")
    posts = cursor.fetchall()
...
def messages_inbox():
    ...
    cursor.execute("SELECT * FROM MESSAGES WHERE (SenderID = %s AND SenderType = %s) OR (ReceiverID = %s AND ReceiverType = %s) ORDER BY SentAt DESC", (my_id, my_role, my_id, my_role))
    ...
    for msg in all_msgs:
        ...
        if f"{p_type}_{p_id}" not in seen:
            ...
            if p_type == 'Brand':
                ...
                res = cursor.fetchone()
                if res: p_name = res['Name']
            inbox.append({'PartnerID': p_id, 'PartnerType': p_type, 'PartnerName': p_name, 'LastMessage': msg['MessageBody'], 'SentAt': msg['SentAt']})
...
def chat(partner_type, partner_id):
    ...
    if request.method == 'POST':
        ...
        if body:
            cursor.execute("INSERT INTO MESSAGES (SenderID, SenderType, ReceiverID, ReceiverType, MessageBody) VALUES (%s, %s, %s, %s, %s)", (my_id, my_role, partner_id, partner_type, body))
            conn.commit()
        return redirect(url_for('chat', partner_type=partner_type, partner_id=partner_id))
    cursor.execute("SELECT * FROM MESSAGES WHERE (SenderID=%s AND SenderType=%s AND ReceiverID=%s AND ReceiverType=%s) OR (SenderID=%s AND SenderType=%s AND ReceiverID=%s AND ReceiverType=%s) ORDER BY SentAt ASC", (my_id, my_role, partner_id, partner_type, partner_id, partner_type, my_id, my_role))
...
def create_post():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO POSTS (AuthorID, AuthorType, Content, ImageURL) VALUES (%s, %s, %s, %s)", (session['user_id'], session['role'].capitalize(), content, image_url))
    conn.commit()
    conn.close()
    if session['role'] == 'influencer': return redirect(url_for('profile'))
    else: return redirect(url_for('brand_dashboard', tab='profile'))
...
def delete_post(post_id):
    ...
    conn.close()
    if session.get('role') == 'influencer': return redirect(url_for('profile'))
    else: return redirect(url_for('brand_dashboard', tab='profile'))
...
def create_campaign():
    ...
    cursor.execute("INSERT INTO CAMPAIGN (CampaignName, CampaignStatus, StartDate, EndDate, BrandID, IndustryID) VALUES (%s, 'Active', %s, %s, %s, %s)", (request.form.get('title'), request.form.get('start_date'), request.form.get('end_date'), session['user_id'], ind_id))
    conn.commit()
    conn.close()
    return redirect(url_for('brand_dashboard', tab='manage'))
...
def apply_campaign(campaign_id):
    ...
    if inf:
        ...
        if not cursor.fetchone():
            cursor.execute("INSERT INTO INFLUENCER_CAMPAIGN (InfluencerID, CampaignID, StartDate, CampaignStatus, Handle, Industry, Age, Gender, Followers) VALUES (%s, %s, %s, 'Pending', %s, %s, %s, %s, %s)", (session['user_id'], campaign_id, date.today(), inf['Handle'], inf['Industry'], inf['Age'], inf['Gender'], inf.get('Followers', 0)))
            conn.commit()
    conn.close()
...
def update_campaign_main_status(campaign_id, action):
    if 'user_id' not in session or session['role'] != 'brand': return redirect(url_for('login'))
    if action not in ['Active', 'Completed']: return redirect(url_for('brand_dashboard'))
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE CAMPAIGN SET CampaignStatus = %s WHERE CampaignID = %s AND BrandID = %s", (action, campaign_id, session['user_id']))
    conn.commit()
    conn.close()
    return redirect(url_for('brand_dashboard', tab='manage'))
...
def public_profile(role, id):
    ...
    cursor.execute("SELECT * FROM POSTS WHERE AuthorID = %s AND AuthorType = %s ORDER BY CreatedAt DESC", (id, role))
    posts = cursor.fetchall()
    conn.close()
    return render_template('public_profile.html', user=user, role=role, id=id, name=name, pic=pic, posts=posts, info="")
...
def profile():
    ...
    cursor.execute("SELECT * FROM POSTS WHERE AuthorID = %s AND AuthorType = 'Influencer' ORDER BY CreatedAt DESC", (session['user_id'],))
